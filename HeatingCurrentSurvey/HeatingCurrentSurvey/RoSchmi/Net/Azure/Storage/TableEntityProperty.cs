...
namespace RoSchmi.Net.Azure.Storage
{
    public class TableEntityProperty
    {
        ...
        public string[] propertyArray()
        {
            ...
            // Setting the Type to "Edm.String" saves some time (no other types are used)
            return new string[4] { StringUtilities.Format("<d:{0} m:type=\"Edm.String\">{1}</d:{0}>", Name, Value, Type), this.Name, this.Value, this.Type };
        }
    }
}
