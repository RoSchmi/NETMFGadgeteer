...
namespace Microsoft.SPOT.Time
{
    /// <summary>
     ...
     public static class FixedTimeService
     {
         ...
         {
             get { return ((status.SyncSourceServer != 0) &&
         ...
         */
         ...
         public static void SetUtcTime(long utcTimeInTicks)
         {
             TimeService.SetUtcTime(utcTimeInTicks);   // also adjusts uptime
         ...
         public static DateTime GetTimeFromNTP(byte[] serverAddress)
         {
             ...
             try
             {
                 ...
                 if (ntpSocket.Poll(timeout * 1000 * 1000, SelectMode.SelectRead))
                 {
                     ...
                     //Debug.Print("NTP time:" + ntpTime.ToString()+" from "+remoteEP.ToString());  

                     ntpTime = ntpTime.AddMinutes(timeZoneOffset);  // universal => local
                     //Debug.Print("Zone time:" + ntpTime.ToString() + " ("+timeZoneOffset+")");

                     if (Settings.AutoDayLightSavings && IsDST(ntpTime))
                         ntpTime = ntpTime.AddMinutes(dstOffset);

                     //Debug.Print("Dst time:" + ntpTime.ToString());
